...
//Let's define the overloaded insertion operator. Remember that it is a non member function
//As a result, this will change the syntax (no scope resolution operator, etc)
//remember that WE DO NOT re-write the word friend when defining
// The purpose here is to cout the contents of the array
//osObject
ostream& operator<<(ostream& osObject, Posts list[]) {
	
	for (int i = 0; i < 10; i++) {
		osObject << list[i].getFollowers() << " ";
	}
	osObject << endl; 
